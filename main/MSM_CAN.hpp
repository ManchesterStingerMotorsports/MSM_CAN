...
namespace MSM_CAN
{
    ...
    inline uint16_t unpack_u16(const uint8_t data[8], uint8_t offset)                // helper function to unpack uint8_t data[8] with a big-endian encoded uint16_t
    {
        if (data == nullptr || offset > 6) return 0;

        return static_cast<uint16_t>(
            (static_cast<uint16_t>(data[offset + 0]) << 8) |
            (static_cast<uint16_t>(data[offset + 1]) << 0));                         // the <<0 is just for stylistic consistency
    }

    inline int16_t unpack_i16(const uint8_t data[8], uint8_t offset)                 // helper function to unpack uint8_t data[8] with a big-endian encoded int16_t
    {
        return static_cast<int16_t>(unpack_u16(data, offset));
    }

    inline uint32_t unpack_u32(const uint8_t data[8], uint8_t offset)                // helper function to unpack uint8_t data[8] with a big-endian encoded uint32_t
    {
        if (data == nullptr || offset > 4) return 0;
    ...
    inline float unpack_float(const uint8_t data[8], uint8_t offset)                 // helper function to unpack uint8_t data[8] with a big-endian encoded float
    {
        const uint32_t bits = unpack_u32(data, offset);
        float value = 0.0f;
